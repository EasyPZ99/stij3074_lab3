...
if (isset($_GET['submit']))
{
    ...
    if ($_GET['submit'] == "cart")
    {
        if ($useremail != "Guest")
        {
            $bookid = $_GET['bookid'];
            $cartqty = "1";
            $stmt = $conn->prepare("SELECT * FROM tbl_carts WHERE user_email = '$useremail' AND item_id = '$bookid'");
            $stmt->execute();
            ...
            if ($number_of_rows > 0)
            {
                ...
                $cartqty = $cartqty + 1;
                $updatecart = "UPDATE `tbl_carts` SET `cart_qty`= '$cartqty' WHERE user_email = '$useremail' AND item_id = '$bookid'";
            ...
            {
                $addcart = "INSERT INTO `tbl_carts`(`user_email`, `item_id`, `cart_qty`) VALUES ('$useremail','$bookid','$cartqty')";
                try
                {
                    $conn->exec($addcart);
...
<body>
    ...
    <div class="w3-top">
        <div class="w3-white w3-xlarge" style="max-width:1200px;margin:auto">
            ...
            <a href="addproduct.php" onclick="w3_close()" class="w3-bar-item w3-button">Add Product</a>
    ...
    <footer class="w3-row-padding w3-padding-32">
        <hr></hr>
         <p class="w3-center">AfiraHerbs&reg;</p>
